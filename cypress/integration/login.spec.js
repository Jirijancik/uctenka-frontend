describe("complete login test!", () => {
  ...
  it("login without credentials", () => {
      ...
      cy.get("[role=alert]").should('have.length', 2).each(($alert) => cy.wrap($alert).should('be.visible'));
      cy.reload();
      cy.visit("http://localhost:3000/dashboard");
  });

  it("login with wrong credentials", () => {
      loginUser(LOGIN, INVALID_PASWORD)
      
      cy.get(".ant-alert-message").should('be.visible');
  });

  it("valid login and logout", () => {
      loginUser(LOGIN, VALID_PASWORD)

      cy.url().should("include", "/dashboard");

      cy.contains('header .ant-space-item > button', 'LOGOUT').click({ force: true });
